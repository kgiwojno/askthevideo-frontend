...
- **Admin enriched event log** (`src/components/admin/EventLog.tsx`, `src/types/admin.ts`): Rewrote the event log to support enriched event objects from the backend. New columns: Tool, Latency (color-coded: green <5s, yellow 5-15s, red >15s), Tokens (formatted as "8.7k/0.5k"). Added TOOL event type. Smart detail rendering per event type (VIDEO shows video_id/duration/chunks, SESSION shows tier/questions/videos). Latency column is sortable (click to toggle asc/desc/off). Summary bar above the table shows avg query latency, total tokens, and cache hit rate.
